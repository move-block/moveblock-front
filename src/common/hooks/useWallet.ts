...
const usePetraWallet = () => {
  ...
  const checkAuthContains = async (moduleAddr : string) => {
    ...
    const response = await fetch(`${API_BASE_URL}/accounts/${address}/contains/${moduleAddr} `);
  ...
  const getSequenceNumber = async () => {
    const address = account?.address;
    const response = await fetch(`https://fullnode.devnet.aptoslabs.com/v1/accounts/${address}`);
